...
public class RoadSegmentEditor : Editor
{
    ...
    {
        if (((RoadSegment)target).settings == null)
        {
            for (int i = 0; i < targets.Length; i++)
            {
                ((RoadSegment)targets[i]).settings = RoadCreatorSettings.GetSerializedSettings();
            }
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Add Extra Mesh"))
        {
            for (int i = 0; i < targets.Length; i++)
            {
                ((RoadSegment)targets[i]).extraMeshes.Add(new ExtraMesh(true, true, (Material)((RoadSegment)targets[i]).settings.FindProperty("defaultExtraMeshMaterial").objectReferenceValue, null, 1, 1, 0));
